...
const char * const BITCOIN_CONF_FILENAME = "paymeal.conf";
const char * const BITCOIN_PID_FILENAME = "paymeald.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "paymeal" is a composite category enabling all Paymeal-related debug output
                if(ptrCategory->count(std::string("paymeal"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\paymealCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\paymealCore
    // Mac: ~/Library/Application Support/paymealCore
    // Unix: ~/.paymealcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "paymealCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/paymealCore";
#else
    // Unix
    return pathRet / ".paymealcore";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        ...
        // Create empty paymeal.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override paymeal.conf
